...
resourceSet
  .uriConverter()
  .map('/models/ecore', Ecore.EcorePackage.get('nsURI'));
...
function buildSimpleModel() {
  var resource = Ecore.Resource.create({
    uri: 'http://www.example.org/models/sample',
  });
  ...
  var UserFriends = Ecore.EReference.create({
    name: 'friends',
    upperBound: -1,
    eType: User,
  });
  ...
  var sample = Ecore.EPackage.create({
    name: 'sample',
    nsPrefix: 'sample',
