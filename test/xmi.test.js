...
describe('#XMI', function () {
      ...
      fs.readFile('./test/models/test3.xmi', 'utf8', function (err, data) {
        ...
        model.load(
          ...
          function (model, err) {
            ...
            var operationParameterDiagnostics = rootClassOperation
              .get('eParameters')
              .at(0);
            assert.strictEqual(
              Ecore.EParameter,
              operationParameterDiagnostics.eClass,
            );
            assert.equal(
              'diagnostics',
              operationParameterDiagnostics.get('name'),
            );
            assert.strictEqual(
              Ecore.EDiagnosticChain,
              operationParameterDiagnostics.get('eType'),
            );

            var operationParameterContext = rootClassOperation
              .get('eParameters')
              .at(1);
            assert.strictEqual(
              Ecore.EParameter,
              operationParameterContext.eClass,
            );
            assert.equal('context', operationParameterContext.get('name'));

            var operationParameterContextType =
              operationParameterContext.get('eGenericType');
            assert.strictEqual(
              Ecore.EGenericType,
              operationParameterContextType.eClass,
            );
            assert.equal(
              2,
            ...
            assert.strictEqual(
              Ecore.EMap,
              operationParameterContextType.get('eClassifier'),
            );
  ...
  describe('Containment feature with upper bound equal to 1:', function () {
    ...
    it('should unset correctly b1', function () {
      ...
      assert.ok(
        b1.eContainer === undefined,
        'eContainer of b1 should be undefined',
      );
      assert.ok(
        b1.eContainingFeature === undefined,
